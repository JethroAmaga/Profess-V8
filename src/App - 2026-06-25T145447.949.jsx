...
function GlobeScene({ radius, dots, markerColor, atmosphereColor, onSpun }) {
  ...
  const SPIN_THRESHOLD = 0.35; // radians of actual user-driven rotation required before counting as "spun"
  return (
    <>
      ...
      <OrbitControls ref={controlsRef} makeDefault enablePan={false} enableZoom={false} minDistance={5} maxDistance={15}
        ...
        onStart={() => {
          ...
          startAzimuthRef.current = controlsRef.current ? controlsRef.current.getAzimuthalAngle() : 0;
        }}
        onChange={() => {
          if (!draggingRef.current || firedRef.current || !controlsRef.current) return;
          const delta = Math.abs(controlsRef.current.getAzimuthalAngle() - startAzimuthRef.current);
          if (delta > SPIN_THRESHOLD) {
